...
PrivateScreen::PrivateScreen(QWidget *parent) :
    QWidget(parent),
...
void PrivateScreen::toHaf(QString gettext)
{
    ...
    {
        if(map->find(gettext[i])==map->end())
            map->insert(gettext[i],1);
   ...
   hafMap->clear();
   if(map->count()==1) {hafWin.displayHafCode(map->begin().key(),"0");return;}
   buildHafCode(parent,"");
